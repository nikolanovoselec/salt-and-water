...
| `src/components/ui/WaveDivider.astro` | Full-width SVG wave divider between sections. Props: `fill` (wave color, default `#F8F5EF`), `flip` (boolean, flips vertically via `scaleY(-1)` for wave-out effect), `class`. Height is fluid: `clamp(40px, 6vw, 80px)`. Used on the homepage to bracket the dark navy section (wave-in / wave-out pair). The sunset CTA section uses an inline SVG wave at the top (same path and dimensions, `fill: var(--color-bg)`, `scaleY(-1)`) with `margin-top: -1px` to eliminate subpixel gaps. |

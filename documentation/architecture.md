...
| `src/components/ui/MiniCollage.astro` | Compact infinite-scroll photo strip. Props: `images: Array<{ src: string; alt: string }>` (required), `speed` (animation duration in seconds, default 20), `reverse` (boolean, default `false` — when `true`, applies `.mini-collage--reverse` which sets `animation-direction: reverse` so the strip scrolls right instead of left), `showCaptions` (boolean, default `false` — when `true`, renders each image's `alt` text as an italic caption below the photo in `.mini-collage__caption`). Renders a WCAG carousel region. Hover pauses animation. Strips with fewer than 2 images fall back to a static single image. Used on apartment detail pages (interior photos), the Food & Drink page (dual opposing strips), and the gallery page (captions enabled). |
...
| `/:locale/galerija` | `src/pages/[locale]/galerija.astro` | Gallery — 139 island photos served as alternating infinite-scroll `MiniCollage` strips (14 strips: 13 of 10 + 1 of 9); strips alternate direction (`reverse` prop); `showCaptions` enabled so poetic Croatian captions appear below each photo; photos shuffled deterministically by locale seed; poetic Croatian captions assigned by rotation; `HeroSimple` with intro text; linked from main nav |
| `/:locale/dolazak` | `src/pages/[locale]/dolazak.astro` | Getting Here — CMS-only: queries dedicated `dolazak` collection, sorted by `sort_order`; each entry maps to one alternating content-row section via `title`, `body`, and `image` fields; a hardcoded address block (`Fratarsko 3, 23271 Ždrelac, Croatia`) with Google Maps and Apple Maps deep-link buttons is always rendered below the CMS sections; renders no content rows if CMS entries absent |
...
| `/:locale/hrana` | `src/pages/[locale]/hrana.astro` | Food & Drink — CMS-only: queries the first entry of the `hrana` collection for the current locale; page-hero + single description section sandwiched between two `MiniCollage` strips; gallery photos are split at the midpoint — the first half scrolls left (default), the second half scrolls right (`reverse` prop); strips are hidden when fewer than 2 photos are available; localized in all 4 locales; linked from homepage triptych |

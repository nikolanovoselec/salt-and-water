...
| `/:locale/hrana` | `src/pages/[locale]/hrana.astro` | Food & Drink — CMS-only: queries dedicated `hrana` collection, sorted by `sort_order`; page-hero + alternating content-row layout; renders no content rows if CMS entries absent; localized in all 4 locales; linked from homepage triptych |

...
| `src/components/home/Hero.astro` | Hero carousel — 7 island photos served from R2 via `/api/img/:uuid.ext`, crossfade (8s CSS transition for long dreamy crossfade), continuous zoom animation (`heroZoom` keyframe: 12s ease-in-out infinite alternate, scale 1→1.1 with -1%/-1% translate, runs continuously on ALL slides so crossfade reveals mid-motion), auto-advance every 10 s, dot navigation, hover-pause; implemented as `is:inline` script |
...
| `gallery_captions` | ~228 (57 × 4 locales) | Poetic photo captions for the gallery page. Each entry: `locale`, `caption` (text), `sort_order`. Queried via `getLocalizedCollection` on the gallery page; captions assigned to photos by rotating index. Seeded via `.emdash/seed.json`. |

...
| `src/components/shell/` | Navigation (desktop nav + mobile hamburger menu with `is:inline` script to avoid Astro bundler stripping; sailboat logo `public/logo.png` rendered at 28px left of the brand name — white on hero, original on scroll), Footer (logo at 36px, white-inverted), LanguageSwitcher, WhatsAppButton, StickyMobileCTA. Mobile menu renders the logo at 48px centered above the nav links (white-inverted). |

...
| `/:locale/galerija` | `src/pages/[locale]/galerija.astro` | Gallery — 138 island photos served as alternating infinite-scroll `MiniCollage` strips (14 strips: 13 of 10 + 1 of 8); strips alternate direction (`reverse` prop); `showCaptions` enabled on all strips so poetic Croatian captions render as visible overlay text over each photo (and also serve as alt text); photos shuffled deterministically by locale seed; `HeroSimple` with intro text; linked from main nav |

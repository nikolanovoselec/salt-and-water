...
| `src/components/shell/` | Navigation (desktop nav + mobile hamburger menu with `is:inline` script to avoid Astro bundler stripping; sailboat logo `public/logo.png` rendered at 28px left of the brand name — white on hero, original on scroll), Footer (text-only brand name, no logo), LanguageSwitcher, WhatsAppButton, StickyMobileCTA. Mobile menu contains no logo — nav links, language picker, and CTA button only. |

...
| `/:locale/apartmani/:slug` | `src/pages/[locale]/apartmani/[slug].astro` | Apartment detail — hero image, breadcrumbs, interior photo collage (`ScrollCollage` at speed 25, sourced from `gallery` CMS field), description, meta grid (sleeps/bedrooms/size/beach distance), price card, amenity list; fetches entry directly by slug via `getEmDashEntry("apartments", slug)` with no locale filtering (locale is applied in rendering, not in lookup); redirects to listing on missing slug |
...
| `/:locale/vodic` | `src/pages/[locale]/vodic.astro` | Local Guide — merges `guide` collection entries (sorted by `sort_order`) and `editorial` entries with `page_key === "vodic"` (sorted by `sort_order`) into a unified list; rendered as a responsive card grid (1 col mobile, 2 col ≥640px, 3 col ≥1024px) where each card is a 4:3 image with a text overlay and hover zoom; localized in all 4 locales |

...
- All images served via Worker route (`/api/img/{key}`, `UUID.ext` keys) from private R2 — served as-is, no edge resizing or format conversion
...
Cloudflare Access authentication for admin panel (zero-trust, JWT validation via `CF_ACCESS_AUDIENCE` AUD tag, auto-provisioned admin users). Cloudflare Turnstile (managed mode, always visible) + honeypot on all forms. Rate limiting: max 5 POST requests to inquiry endpoint per IP per 10 minutes via Cloudflare WAF Rate Limiting Rules (Turnstile score is primary defense). CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, HSTS (1 year, includeSubDomains), Cross-Origin-Opener-Policy (same-origin-allow-popups), Cross-Origin-Resource-Policy (same-origin) headers. Permissions-Policy restricts: camera, microphone, geolocation, accelerometer, gyroscope, magnetometer, midi, payment, usb. No hardcoded secrets — all via Workers secrets. Inquiry data persisted to D1 before email attempt. Server-side Turnstile verification. Server-side availability revalidation. Input sanitization (no HTML injection, email header injection prevented). EXIF GPS data retained in R2 originals; originals are served as-is via the Worker route, so EXIF metadata may be present in served images. Security contact disclosed via `/.well-known/security.txt` (RFC 9116, managed via Cloudflare dashboard).

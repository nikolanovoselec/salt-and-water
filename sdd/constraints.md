...
Cloudflare Workers runtime with Astro 6 and Emdash CMS as an Astro integration. D1 for database, R2 for media storage. Single Worker deployment. **Custom domain:** `apartmani.novoselec.ch` (configured as Cloudflare custom domain route in `wrangler.jsonc`). Web-standard APIs only (fetch, crypto.subtle, Request/Response, URL, TextEncoder). No Node.js-specific APIs (fs, child_process, net). Requires `nodejs_compat` compatibility flag for Emdash dependencies. Configuration via `wrangler.jsonc`. **Images served via Worker route (`/api/img/{key}`) fetching from private R2 and returned as-is** — no on-upload processing, no edge resizing, no format conversion. R2 object keys are normally `UUID.ext` format (the format produced by `POST /admin/api/upload-url` and the Emdash media library); the 7 hard-coded hero carousel keys are bare extension-less UUIDs (legacy, predating the `UUID.ext` standard). The route accepts both because content type is read from R2 object metadata, not from the key suffix. All date/time logic uses `Europe/Zagreb` timezone. **Vite build limitation:** `cloudflare:workers` module cannot be imported at Vite build time — Emdash plugins that access Worker env bindings via dynamic import fail during the Astro build. Runtime-only access to Worker bindings must use API routes or middleware, not Emdash plugin hooks. **Workaround:** Emdash plugins that need secrets (e.g., Resend API key) read them from Emdash's plugin KV store (`ctx.kv`) at runtime instead of from Worker env bindings. **Env access pattern (Astro v6):** API routes access Cloudflare bindings via `import { env as _env } from "cloudflare:workers"` (module-scoped, not extracted from Astro locals). The `@astrojs/cloudflare` adapter marks this import as external; it resolves at Worker runtime. The `cloudflare:workers` `env` export is typed as `Record<string, unknown>`; the project provides type safety by declaring an `Env` interface in `src/env.d.ts` (listing all bindings: `DB`, `MEDIA`, `RESEND_API_KEY`, etc.) and casting at the call site (`const env = _env as unknown as Env;`). API routes import the `Env` type explicitly. **Non-secret configuration** (`ADMIN_EMAILS`, `TURNSTILE_SITE_KEY`, `CLOUDFLARE_ACCOUNT_ID`, `R2_BUCKET_NAME`, `CF_ACCESS_AUDIENCE`) lives in `wrangler.jsonc` `vars` block as plaintext. Secrets (`JWT_SECRET`, `EMDASH_AUTH_SECRET`, `TURNSTILE_SECRET_KEY`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`) are set via `wrangler secret put`. `RESEND_API_KEY` is stored in Emdash's plugin KV store (key: `resend_api_key`) because the Resend email plugin is bundled at Vite build time and cannot access `cloudflare:workers` bindings. `CF_ACCESS_AUDIENCE` is the Cloudflare Access Application AUD tag — a public identifier (not a secret), used by the `access()` plugin to validate `CF_Authorization` JWTs. `CLOUDFLARE_ACCOUNT_ID` is a plaintext var needed for generating R2 presigned upload URLs. **Email sender domain:** All transactional emails (owner inquiry notifications, legacy Magic Link codes) use `noreply@graymatter.ch` as the From address via Resend. **Admin authentication** handled by Cloudflare Access (not Resend).
...
- All images served via Worker route (`/api/img/{key}`) from private R2 — served as-is, no edge resizing or format conversion. Keys are R2 object identifiers in either `UUID.ext` form (standard, generated by upload-url and Emdash media library) or bare extension-less UUID form (legacy hero carousel keys); content type is read from R2 object metadata so both work.

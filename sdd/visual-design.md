...
- **Acceptance Criteria:**
  ...
  - **Homepage waves (2 placements):** (1) cream-to-dark before apartments section, (2) dark-to-cream after apartments section. ~~(3) cream-to-dark before sunset CTA section~~ — **Removed:** the wave between the experience triptych and the sunset CTA section was removed; the CTA section now transitions directly from the experience triptych section without a wave divider. Each wave's `fill` color must exactly match the background of the section it transitions INTO. The `bg` (background) must match the section it sits ON TOP OF. No visible color gaps or mismatches between wave and adjacent sections.
...
- **Acceptance Criteria:**
  ...
  - `showCaptions` prop (default false): when enabled, renders each photo's alt text as an overlay caption positioned absolute at the bottom of the photo. Caption styling: sans-serif (`var(--font-sans)`), extra-small font, normal weight (400), letter-spacing 0.02em, white text at 90% opacity, with a bottom-to-top gradient background (`rgba(0,0,0,0.45)` to transparent) for legibility over any image. `pointer-events: none` so the overlay does not interfere with hover pause. Currently enabled only on the gallery page (REQ-SF-8) to display poetic Croatian captions over photos.

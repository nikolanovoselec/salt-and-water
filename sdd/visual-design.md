...
- **Acceptance Criteria:**
  ...
  - **Last section before footer (`.section--footer-clear`):** The last content section on every page uses the `.section--footer-clear` CSS utility class. The class applies `padding-bottom: calc(var(--space-section) + clamp(50px, 8vw, 100px))` — standard section spacing plus the footer wave height — ensuring the footer's top-edge wave SVG (REQ-SF-6) never overlaps page content. Applied universally: every page's final section or container must carry this class. This replaces all previous inline `padding-bottom` overrides and `style` attributes for footer wave clearance.

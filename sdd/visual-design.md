...
- **Acceptance Criteria:**
  ...
  - **Last section before footer (`.section--footer-clear`):** CSS utility class that applies `padding-bottom: calc(var(--space-section) + clamp(50px, 8vw, 100px))` — standard section spacing plus the footer wave height — ensuring the footer's top-edge wave SVG (REQ-SF-6) never overlaps interactive content (CTA buttons, inquiry forms). Applied selectively to pages where a call-to-action, interactive element, or single-section layout sits in the final section near the footer wave: homepage CTA section, apartments listing CTA section, kontakt page, dolazak (Getting Here) dark address section, and o-nama (about) page. The about page has a single content section between hero and footer, requiring explicit clearance to prevent the footer wave from overlapping the host story text. Editorial and content pages with sufficient section depth (vodic, hrana, aktivnosti, plaze, galerija, faq, privatnost, apartment detail) use standard section padding — the footer wave transitions naturally within the existing padding zone without obscuring content.
...
- **Status:** Implemented — all subpages have real Croatian hero photos served from R2 via `/api/img/{key}` (`UUID.ext` keys); inline SVG waves on all custom heroes; no non-Croatian imagery remains

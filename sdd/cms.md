...
- **Acceptance Criteria:**
  ...
  - **Seed data (`seed/seed.json`):** Collections across 4 locales: `apartments` (2 listings: Lavanda 4-pax, Tramuntana 2-pax with full field set), `faqs` (categorized Q&A), `testimonials` (guest quotes with country metadata), `editorial` (homepage sections: why-pasman, zdrelac, apartments, cta + privacy page). Dead collections removed from seed: guide, posts, pages, homepage. All entries have per-locale variants (hr, de, sl, en).
  - **Dedicated collection seeding (critical):** Editorial pages now use dedicated CMS collections (not `editorial` with page_key filter). Each must be seeded in all 4 locales: `vodic` (8 sections — includes merged Ždrelac content as sections 1-4), `hrana` (4 sections), `aktivnosti` (4 sections), `plaze` (4 sections), `dolazak` (3 sections), `about` (1 section). Homepage sections and privacy remain in the `editorial` collection. Hardcoded fallback content has been removed from all editorial pages — pages render empty sections if CMS entries are missing. Complete seeding is a prerequisite for a functional site.

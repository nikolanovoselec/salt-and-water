...
- **Verification:** Deploy fresh instance, apply D1 SQL migrations, verify complete site renders in all 4 locales with no empty sections (all editorial collections have CMS entries), verify placeholder badges
- **Status:** Partial — editorial pages migrated to dedicated CMS collections (vodic, hrana, aktivnosti, plaze, dolazak, about). Vodic expanded to 8 sections (Ždrelac content merged as sections 1-4). Zdrelac standalone page deleted; content preserved in vodic collection. HR entries seeded via D1 SQL for all pages. DE/SL/EN entries now present and polished for `aktivnosti`, `hrana`, and `dolazak` collections (27 D1 entries across 3 locales — Croatianisms removed, tone aligned per REQ-I18N-6). Remaining DE/SL/EN gaps: `vodic`, `plaze`, `about` collections. All 48 Pexels stock photo URLs replaced with real R2 photos in live D1. `seed/seed.json` and seed API endpoint removed; live D1 is the source of truth. **Risk: pages with missing CMS entries render empty sections (no fallback) so incomplete seeding in non-HR locales for vodic/plaze/about still produces visibly broken pages.**

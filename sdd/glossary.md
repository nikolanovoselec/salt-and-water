...
| **section--footer-clear** | CSS utility class applied to the last content section on every page. Provides `padding-bottom: calc(var(--space-section) + clamp(50px, 8vw, 100px))` (standard section spacing plus footer wave height), ensuring the footer's top-edge wave SVG does not overlap page content. Counterpart to `section--wave-in` (top-edge clearance). Replaces per-page inline `padding-bottom` overrides. |

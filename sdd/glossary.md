...
| **section--footer-clear** | CSS utility class applied to the last content section on pages where a CTA or interactive element sits near the footer wave. Provides `padding-bottom: calc(var(--space-section) + clamp(50px, 8vw, 100px))` (standard section spacing plus footer wave height), ensuring the footer's top-edge wave SVG does not overlap interactive content. Used on: homepage CTA, apartments listing CTA, kontakt, dolazak. Editorial/content pages use standard section padding instead. Counterpart to `section--wave-in` (top-edge clearance). |

...
The previous pass (Post-f0993c1) corrected only the JSDoc claim inside `/api/img/[key].ts`. It did not touch `sdd/` files. This pass closes the matching gap inside the spec — three spec locations were still asserting `UUID.ext` exclusively, contradicting the actual `Hero.astro` source.

### Source verification
- `src/components/home/Hero.astro` lines 12-18: 7 entries of the form `/api/img/{bare-uuid}` with no file extension. These are the only bare-UUID image references in the codebase. All other site imagery (gallery, subpage heroes, triptych, editorial, apartment interiors) was uploaded via the Emdash media library and uses `UUID.ext`.
- `src/pages/api/img/[key].ts` (commit b85cb9f): JSDoc now documents both forms explicitly. Route implementation unchanged — it always accepted both, because Emdash storage looks up by exact key string and R2 returns the stored content type from object metadata.
- `documentation/decisions/README.md` AD15 (commit b85cb9f): now states "Keys generated by `POST /admin/api/upload-url` and the Emdash media library use `UUID.ext` format ... The hard-coded hero carousel keys in `Hero.astro` are an exception: they are extension-less UUIDs ... predating the `UUID.ext` standard."

### Requirements updated this pass
- **REQ-PERF-1** (Image Serving Pipeline) — Acceptance criterion #2 (image delivery via Worker route) previously stated `Object keys use `UUID.ext` format (file extension included)` as an absolute. Rewrote to specify both forms — `UUID.ext` (standard, from `POST /admin/api/upload-url` + Emdash media library) and bare extension-less UUIDs (legacy hero carousel keys) — and noted explicitly that the route accepts both because content type comes from R2 object metadata. Status line also updated: removed the absolute "with `UUID.ext` keys" claim and replaced with "Most keys use `UUID.ext` format; the 7 hard-coded hero carousel keys are bare extension-less UUIDs (legacy from before the `UUID.ext` standard)".
- **REQ-SF-1** (Hero Section) — "Image source" criterion previously did not specify the key shape used by the hero carousel and the "Future enhancement" line incorrectly called the hero keys "hardcoded local paths" (a stale phrasing from before the eb5def7 R2 migration). Added an explicit note that the 7 hero carousel keys are hard-coded bare extension-less UUIDs (legacy, from before the `UUID.ext` standard) and that the route accepts both forms because content type comes from R2 object metadata. Reworded the future-enhancement line from "hardcoded local paths" to "hardcoded keys" with the forward-looking note that future CMS-managed hero selections will use the standard `UUID.ext` format produced by the upload pipeline.
- **CON-STACK** (Technology Stack) — Previously stated `R2 object keys use `UUID.ext` format (extension included)` as an absolute. Rewrote to call out that `UUID.ext` is the standard format (from `POST /admin/api/upload-url` + Emdash media library), the 7 hard-coded hero carousel keys are a bare-UUID legacy exception, and the route accepts both because content type is read from R2 object metadata.
- **CON-PERF** (Performance Budget) — Performance bullet previously listed `UUID.ext` keys as an absolute. Removed the absolute and added the same dual-form explanation matching CON-STACK and REQ-PERF-1.

### Requirements deliberately not changed (already accurate in their narrower context)
- **REQ-CMS-2** (Media Library): Says `Object keys use `UUID.ext` format` — accurate in context because REQ-CMS-2 describes the upload pipeline output (presigned PUT via `/admin/api/upload-url` → Emdash media library), and that pipeline does always produce `UUID.ext`. The bare-UUID exception lives in REQ-SF-1's hard-coded carousel, not in the upload pipeline.
- **REQ-VD-12** (Subpage Hero Pattern), **REQ-VD-14** (Unique Imagery Per Page): Status lines mention "`UUID.ext` keys" — accurate because subpage heroes and content imagery (gallery, hrana, aktivnosti, plaze, etc.) are uploaded via the Emdash media library and do use `UUID.ext`. Only the homepage hero carousel is the exception.
- **REQ-CMS-6** (Preloaded Content), **REQ-SF-8** (Gallery Page): Both mention `UUID.ext` — accurate; preloaded content and the 142 gallery photos are seeded/uploaded via the upload pipeline.
- **CON-MEDIA**: No `UUID.ext` claim. No change needed.
- **Glossary, actors, dependency graph**: No changes triggered by this pass.

### Quality issues fixed
- **Three load-bearing absolute claims softened to match reality**: REQ-PERF-1 AC + status, CON-STACK, CON-PERF all previously asserted `UUID.ext` exclusively. Each now correctly distinguishes the standard upload-pipeline format from the legacy hero exception, matching the source code that has existed since the original R2 migration. This brings the spec into convergence with the docs/ADR, which were updated in commit b85cb9f.
- **REQ-SF-1 "hardcoded local paths" misstatement corrected**: the future-enhancement note implied hero keys were local file paths (`/photos/...`), but that migration completed in commit eb5def7. Hero keys have been R2 keys (just bare-UUID R2 keys) ever since. Corrected to "hardcoded keys".

### Result
**Spec is converged with reality at commit b85cb9f.** Zero requirements added, zero deprecated. Four spec locations updated (REQ-PERF-1, REQ-SF-1, CON-STACK, CON-PERF) to acknowledge the dual key-form reality of `/api/img/:key`. The spec, the docs (`documentation/architecture.md`), the ADR (`AD15` in `documentation/decisions/README.md`), and the source (`Hero.astro` + `[key].ts` JSDoc) now all agree: standard uploads produce `UUID.ext`; the 7 hard-coded hero carousel keys are bare-UUID legacy; both work because content type comes from R2 object metadata.
